...
import MFL_TopFullLogo from '@salesforce/resourceUrl/MFL_TopFullLogo';

export default class SsThirdPartyApplicationForm extends LightningElement {
    // Input properties from Flow
    @api recordId;       // Original Account Id from Flow
    ...
    @api parentAccountId; // Deprecated - kept for backward compatibility
     ...
     */
    connectedCallback() {
        // Try to get recordId from URL if not provided by Flow
        if (!this.recordId) {
            this.recordId = this.getUrlParameter('recordId');
        }
        
        // Set parentAccountId to the same value as recordId for Flow compatibility
        this.parentAccountId = this.recordId;
    ...
    async createAccount(accountData) {
        try {
            ...
            this.thirdPartyId = result; // result is now the ID string directly
            this.parentAccountId = this.recordId; // Ensure parent account ID is set for Flow
    ...
    validateInputs() {
        ...
        // Remove all field validations for now - just allow submission
        // if (!this.recordTypeId) {
        //     this.setError('Record Type ID is required');
        //     return false;
        // }

        // if (!this.recordId) {
        //     this.setError('Record ID (Parent Account ID) is required');
        //     return false;
        // }
