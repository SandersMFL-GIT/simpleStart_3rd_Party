...
export default class ConflictAlertModal extends NavigationMixin(LightningModal) {
     ...
     */
    get displayMessage() {
        if (!this.message) return 'Potential conflict detected on this account.';
    ...
    get detailsRich() {
        ...
        // Format lines with bold labels and clean up the 'true' values
        const htmlLines = lines
            .filter(line => line.trim()) // Remove empty lines
            .map(line => {
                // Replace ': true' with just ':'  for cleaner display
                const cleanLine = line.replace(/:\s*true\s*$/, '');
                // Make the label bold (text before colon)
                return cleanLine.replace(/^([^:]+:)/, '<strong>$1</strong>');
            });
     ...
     */
    get hasDetails() {
        return this.detailsRich.length > 0;
    }
    ...
    buildMatchSubtitle(match) {
        ...
        if (match.Phone || match.phone) {
            parts.push(match.Phone || match.phone);
        }
        
        if (match.Email || match.email) {
            parts.push(match.Email || match.email);
        }
        
        if (match.Type || match.type) {
            parts.push(match.Type || match.type);
        }
